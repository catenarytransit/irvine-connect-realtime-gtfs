...
pub async fn load_gtfs() -> Result<GtfsData, Box<dyn std::error::Error + Send + Sync>> {
    ...
    for (trip_id, service_id, block_id) in trips_raw {
        ...
        trips_by_block.entry(block_id.clone()).or_default().push(idx);
...
fn parse_stops(archive: &mut zip::ZipArchive<Cursor<&[u8]>>) -> Result<HashMap<String, Stop>, Box<dyn std::error::Error + Send + Sync>> {
...
fn parse_trips(archive: &mut zip::ZipArchive<Cursor<&[u8]>>) -> Result<Vec<(String, String, String)>, Box<dyn std::error::Error + Send + Sync>> {
...
fn parse_stop_times(archive: &mut zip::ZipArchive<Cursor<&[u8]>>) -> Result<HashMap<String, Vec<StopTime>>, Box<dyn std::error::Error + Send + Sync>> {
